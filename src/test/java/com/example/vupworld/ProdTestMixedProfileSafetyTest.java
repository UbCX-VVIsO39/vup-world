...
@SpringBootTest(properties = {
        "spring.datasource.url=jdbc:h2:mem:vupworld-prod-test-mixed;MODE=MySQL;NON_KEYWORDS=DAY,LOCKED;DATABASE_TO_LOWER=TRUE;CASE_INSENSITIVE_IDENTIFIERS=TRUE;DB_CLOSE_DELAY=-1",
...
class ProdTestMixedProfileSafetyTest {

    @Autowired
    private MockMvc mockMvc;

    @Test
    void prodProfileWinsOverTestForConfigCheckAndDevEndpoints() throws Exception {
        mockMvc.perform(get("/api/system/config-check"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                // 未登录访问 config-check 时 devTools 被裁剪为 null；prod 优先于 test 由 dev 接口返回 DEV_TOOL_DISABLED 验证。
